...
class ImageUploadService {
	async requestCameraPermissions(): Promise<boolean> {
		try {
			...
			console.error(
				"[ImageUploadService] Error al solicitar permisos de cámara:",
				error,
			);
			return false;
		}
	...
	async requestMediaLibraryPermissions(): Promise<boolean> {
		try {
			...
			console.error(
				"[ImageUploadService] Error al solicitar permisos de galería:",
				error,
			);
			return false;
		}
	...
	async uploadImage(
		...
		try {
			...
			console.log(
				"[ImageUploadService] Blob obtenido:",
				blob.size,
				"bytes",
				blob.type,
			);
			...
			const _fileExt = options.fileName.split(".").pop() || "jpg";
