...
export default function ValidateRedemptionScreen() {
	...
	const [qrInfo, setQrInfo] = useState<Record<string, unknown> | null>(null);
			...
			toast.show({
				...
				render: ({ id }) => (
					<Toast nativeID={`toast-${id}`} action="success" variant="solid">
						...
						<ToastDescription>
							Puntos descontados correctamente.
						</ToastDescription>
					</Toast>
				),
			});
			...
			toast.show({
				...
				render: ({ id }) => (
					<Toast nativeID={`toast-${id}`} action="error" variant="solid">
						...
						<ToastDescription>
							{err.message || "Saldo insuficiente o error de red"}
						</ToastDescription>
					</Toast>
				),
			});
	...
	if (!qrInfo)
		return (
			<Box className="flex-1 bg-white center">
				<Spinner color="#2F4858" />
			</Box>
		);
	...
	return (
		<AppLayout
			showHeader={true}
			headerVariant="back"
			headerTitle="Validar canje de recompensa"
			showNavBar={false}
		>
			<VStack space="lg" className="p-2 pt-6">
				...
				<Box className="bg-blue-50 p-4 rounded-lg border border-blue-100">
					<Text className="text-gray-800 font-bold text-lg mb-1">
						Puntos a descontar: {qrInfo.pointsCost}
					</Text>
					<Text className="text-gray-600 text-sm">
						Al confirmar, se restarán estos puntos del cliente permanentemente.
					</Text>
					<Text className="text-gray-600 text-sm mt-2">
						Infórmele al cliente que se le descontarán {qrInfo.pointsCost}{" "}
						puntos antes de confirmar.
					</Text>
				</Box>

				{/* Instrucciones Dinámicas */}
				<Box className="mt-6">
					<Heading size="md" className="text-[#2F4858] mb-4 font-bold">
						Instrucciones
					</Heading>
					...
					<VStack space="md">
						<HStack space="sm">
							...
							<Text className="text-gray-600 flex-1 leading-5">
								Confirma visualmente que la{" "}
								<Text className="font-bold">{qrInfo.rewardName}</Text> es el
								producto/servicio que se está entregando al cliente.
							</Text>
						</HStack>
						...
						<HStack space="sm">
							...
							<Text className="text-gray-600 flex-1 leading-5">
								Dirígete a la caja (TPV) y registra el producto. Aplica el
								descuento o marca el producto como $0.00 según el procedimiento
								del local.
							</Text>
						</HStack>
						...
						<HStack space="sm">
							...
							<Text className="text-gray-600 flex-1 leading-5">
								Entrega el producto/recompensa al cliente junto con su recibo de
								caja.
							</Text>
						</HStack>
						...
						<HStack space="sm">
							...
							<Text className="text-gray-600 flex-1 leading-5">
								Una vez entregado, presiona "Confirmar Entrega" para realizar el
								descuento pertinente de los puntos al usuario. ¡La transacción
								ha terminado!
							</Text>
						</HStack>
					</VStack>
				...
				<HStack space="md" className="mt-8 pb-8">
					...
					>
						<ButtonText className="text-gray-600 font-medium">
							Cancelar ✕
						</ButtonText>
					</Button>
					...
					>
						{isRedeeming ? (
							...
							<ButtonText className="font-medium">Confirmar ✓</ButtonText>
						)}
					</Button>
				</HStack>
			</VStack>
