...
import { AlertCircleIcon, EditIcon, InfoIcon, TrashIcon } from "lucide-react-native";
...
export default function RewardDetailScreen() {
	...
	const { deleteReward, isDeleting, deleteSuccess, deleteError } = useReward(ownBusinessId);
	...
	if (error || !reward) {
		return (
			<AppLayout showHeader={true} showNavBar={false} headerVariant="back">
				<FeedbackScreen variant="error" icon={AlertCircleIcon} title="Error" description="No se encontró la recompensa." />
			</AppLayout>
		);
	}
	...
	return (
		<AppLayout showHeader={true} showNavBar={false} scrollable={true} headerVariant="back" headerTitle="Detalle de recompensa">
			<VStack space="lg" className="pb-6 px-1">

				{isCustomer && business && (
					<HStack space="md" className="items-center justify-between px-2 mb-2">
						<Heading size="lg" className="text-primary-500 flex-1">{business.name}</Heading>
						<VStack space="xs" className="items-end">
							<Text className="text-typography-500 text-xs">Tus puntos actuales</Text>
							<Badge action="info" variant="solid" size="lg"><BadgeText>{currentPoints} puntos</BadgeText></Badge>
						</VStack>
					</HStack>
				)}
				...
				<Box className="w-full aspect-square bg-gray-100 rounded-lg overflow-hidden">
					{reward.imageUrl ? (
						<Image source={{ uri: reward.imageUrl }} alt={reward.name} className="w-full h-full" resizeMode="cover" />
					) : (
						<Box className="w-full h-full justify-center items-center"><Text className="text-gray-400">Sin imagen</Text></Box>
					)}
				</Box>

				<Heading size="xl" className="text-[#2F4858]">{reward.name}</Heading>
				<Text className="text-gray-700 leading-6">{reward.description || "Sin descripción disponible"}</Text>
					...
					<Box className="mt-4">
						<Box className="border border-blue-200 bg-blue-50 rounded-lg p-4 mb-6">
							<HStack space="sm" className="items-start">
								...
								<Text className="text-info-800 text-sm flex-1">
									El cobro de los puntos se realizará automáticamente una vez que el código QR sea escaneado por el personal del negocio.
								</Text>
							</HStack>
						</Box>
						...
						{!canRedeem && (
							<Text className="text-gray-500 text-sm text-center mt-2">
								Te faltan {reward.pointsRequired - currentPoints} puntos para canjear.
							</Text>
						)}
					</Box>
				)}
				...
				{isOwner && (
					<VStack space="md" className="mt-5">
						<Button variant="outline" className="rounded-lg border-[#2F4858]" size="lg" onPress={handleEditPress}>
							<Icon as={EditIcon} className="text-primary-500 mr-2" size="sm" />
							<ButtonText className="text-primary-500">Editar recompensa</ButtonText>
						</Button>

						<Button variant="outline" className="rounded-lg border-red-500" size="lg" onPress={() => setShowDeleteModal(true)} isDisabled={isDeleting}>
							<Icon as={TrashIcon} className="text-red-500 mr-2" />
							<ButtonText className="text-red-500">{isDeleting ? "Eliminando..." : "Eliminar recompensa"}</ButtonText>
						</Button>
					</VStack>
				)}

				{isOwner && deleteError && (
					<Box className="bg-red-50 p-3 rounded-lg mt-2"><Text className="text-error-500">{deleteError.message}</Text></Box>
				)}
			</VStack>
			...
			{isOwner && (
				<Modal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)}>
					<ModalBackdrop />
					<ModalContent>
						<ModalHeader><Heading size="lg">¿Eliminar recompensa?</Heading></ModalHeader>
						<ModalBody><Text>La recompensa "{reward.name}" dejará de estar visible.</Text></ModalBody>
						<ModalFooter>
							<Button variant="outline" onPress={() => setShowDeleteModal(false)} className="mr-2"><ButtonText>Cancelar</ButtonText></Button>
							<Button className="bg-red-600" onPress={handleDelete}><ButtonText className="text-white">Eliminar</ButtonText></Button>
						</ModalFooter>
					</ModalContent>
				</Modal>
			)}
