...
import {
	...
	useSelectWinner,
} from "@/src/presentation/hooks/useRaffleWinner";
...
const ParticipantCard = ({
	...
	ticketsCountMap: Record<string, number>;
}) => {
...
export default function SelectWinnerScreen() {
	...
	const startRoulette = () => {
		...
		const winnerIndexInExtendedList =
			targetBandIndex * numParticipants + finalIndex;
		const targetScrollPosition =
			winnerIndexInExtendedList * CARD_SIZE - CENTER_OFFSET;
		...
		const runStep = () => {
			...
			if (progress < 1) {
				const easedProgress = 1 - (1 - progress) ** 5; // Ease-out Quintic
	...
	return (
		<AppLayout
			...
			showNavBar={false}
			headerVariant="back"
			headerTitle="Seleccionar ganador"
		>
			<VStack className="flex-1 p-4 justify-between pb-10">
				...
				<Box className="flex-1 justify-center items-center">
					{participants.length === 0 ? (
						...
						<VStack className="items-center w-full">
							...
							<VStack
								className="absolute z-10 top-[-25] items-center"
								style={{ width: "100%" }}
							>
								...
								{winner && (
									<VStack className="items-center mt-6">
										...
										<Heading
											size="xl"
											className="text-[#2F4858] font-extrabold mt-2 text-center"
										>
							...
							{!winner ? ( // 🔥 Mostrar ScrollView SÓLO si no hay ganador final
								...
								</ScrollView> // Mostrar info simple del ganador (sin repetir nombre)
							) : (
								<VStack className="w-full h-40 mt-16 justify-center items-center">
									...
									<Box className="bg-[#2F4858] px-3 py-1 rounded-full mt-2 flex-row items-center">
										...
										<Text className="text-white font-bold">
											{ticketsCountMap[winner.customer.id] ?? 0}
										</Text>
									</Box>
								</VStack>
							)}
				...
				<Box>
					{!winner ? (
						...
						>
							<ButtonText>
								{isSpinning ? "Girando..." : "Empezar ruleta"}
							</ButtonText>
						</Button>
					) : (
						<VStack space="md">
							...
							>
								<ButtonText>
									{isConfirming ? <Spinner color="white" /> : "Confirmar"}
								</ButtonText>
							</Button>
							<Button
								variant="outline"
								onPress={() => setWinner(null)}
								className="border-[#2F4858] rounded-lg h-12"
							>
