...
export const usePromotions = (businessId: string | null) => {
  return useQuery<Promotion[], Error>({
    ...
    queryFn: async () => {
      ...
      console.log('[usePromotions] 📋 Obteniendo promociones para negocio:', businessId);
      ...
      console.log('[usePromotions] ✅ Promociones obtenidas:', data?.length || 0);

      // Convertir de snake_case a camelCase
      const promotions: Promotion[] = (data || []).map((promo: any) => ({
        id: promo.id,
