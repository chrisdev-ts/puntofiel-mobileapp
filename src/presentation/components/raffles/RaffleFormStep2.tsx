			...
			toast.show({
				...
				render: ({ id }) => (
					<Toast nativeID={`toast-${id}`} action="warning" variant="solid">
						...
						<ToastDescription>
							Necesitamos acceso a la galería para subir la imagen.
						</ToastDescription>
					</Toast>
				),
			});
	...
	return (
		<VStack space="lg">
			...
			{Platform.OS !== "web" && (
				<Button
					variant="outline"
					className="bg-white border-[#2F4858] rounded-lg"
					size="lg"
					onPress={takePhoto}
				>
					<Icon as={CameraIcon} className="text-[#2F4858] mr-2" />
					<ButtonText className="text-[#2F4858] font-medium">
			...
			<VStack space="sm" className="mt-8">
				...
				<Button
					...
					onPress={onBack}
					isDisabled={isSubmitting}
					className="border-[#CCCCCC] rounded-lg"
					size="lg"
				>
