...
import { promotionFormSchema, type PromotionFormData } from './PromotionSchema';
...
export const PromotionFormStep1: React.FC<PromotionFormStep1Props> = ({
  ...
  useEffect(() => {
    if (initialData && (initialData.title || initialData.content)) {
      console.log('[PromotionFormStep1] 🔄 Reseteando formulario con datos:', initialData);
      reset(initialData);
    }
  }, [initialData, reset]);
  ...
  return (
    <ScrollView className="flex-1 bg-white" showsVerticalScrollIndicator={false}>
      <VStack className="px-6 py-4">
        <Card className="p-6 rounded-lg bg-white border border-gray-200">
          <VStack className="gap-4">
            <Heading className="text-2xl font-bold">Crear promoción</Heading>
            <Text className="text-gray-600">
              Ingresa la información de la promoción para darla de alta en tu negocio.
            </Text>
            ...
            <FormControl isInvalid={!!errors.title}>
              ...
              {errors.title && (
                <View className="bg-red-50 p-2 rounded mt-1">
                  <Text className="text-red-600 text-sm">{errors.title.message}</Text>
                </View>
              )}
            </FormControl>
            ...
            <FormControl isInvalid={!!errors.content}>
              ...
              {errors.content && (
                <View className="bg-red-50 p-2 rounded mt-1">
                  <Text className="text-red-600 text-sm">{errors.content.message}</Text>
                </View>
              )}
            </FormControl>
            ...
            <FormControl isInvalid={!!errors.startDate}>
              ...
              {errors.startDate && (
                <View className="bg-red-50 p-2 rounded mt-1">
                  <Text className="text-red-600 text-sm">{errors.startDate.message}</Text>
                </View>
              )}
            </FormControl>
            ...
            <FormControl isInvalid={!!errors.endDate}>
              ...
              {errors.endDate && (
                <View className="bg-red-50 p-2 rounded mt-1">
                  <Text className="text-red-600 text-sm">{errors.endDate.message}</Text>
                </View>
              )}
            </FormControl>
