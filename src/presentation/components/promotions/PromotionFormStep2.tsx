...
export const PromotionFormStep2: React.FC<PromotionFormStep2Props> = ({
  ...
  const [displayImage, setDisplayImage] = useState<string | null>(initialImageUrl || null); // ✅ Agregado
  ...
  const handleUploadImage = async () => {
    ...
    try {
      ...
      if (result.success && result.url) {
        console.log('[PromotionFormStep2] ✅ Imagen subida exitosamente:', result.url);
      ...
      <Card className="bg-gray-100 rounded-lg overflow-hidden mb-6 w-full aspect-video justify-center items-center">
        {displayImage ? (
          <RNImage
            ...
            resizeMode="cover"
            onError={(error) => {
              console.error('[PromotionFormStep2] Error cargando imagen:', error);
            }}
          />
        ) : (
